...
- Adds explicit strategy/cost/assumption confirmation with blank numeric inputs, immutable version history and deactivation for future research.
- Displays horizons, benchmark, dated valuation inputs, conditional estimates, monitoring thresholds, stance and stale-version warnings separately from execution prices.
- Reuses the existing HTTP-compatible request ID generator so local non-secure browser contexts cannot crash the form.
- All 272 frontend tests, strict lint/types and 37 final-image browser scenarios pass; repeated build manifests match. Hosted publication pending; live personal strategy remains unconfigured.

## [0.37.0] - 2026-09-17

### Sealed evidence navigation
...
- Adds runtime schema/component tests and real-API risk browser scenarios. All 269 tests, 30 final-image browser cases, repeated clean builds and protected PR #10 CI pass; shipped as `ef1fd2c`.
...
- Adds real-API browser proof of Gemini chat, Grok/Google role tests, persistence and optimistic-concurrency errors.
- Passes 261 tests, zero-warning production lint, unchanged 131-warning test ceiling and protected PR #6 hosted validation.
...
- Removed fake authentication tokens, authorization headers, admin gates, and
  the unused auth translation namespace.
- Removed the misleading model/credit settings UI and the obsolete
  `/api/models` client surface; Agent mode selection remains available.
- Removed the unused React Router dependency, dead Nginx production image,
  placeholder tests, obsolete credit strings, and one-off browser probes.
- Replaced the Kubernetes CronJob panel with a local manual analysis runner.
- Simplified the health dashboard to real local database metrics.
- Updated current documentation and validated TypeScript, ESLint, and the full
  Vitest suite.
