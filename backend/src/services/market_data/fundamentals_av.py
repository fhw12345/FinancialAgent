...
async def fetch_company_overview(mixin: Any, symbol: str) -> dict[str, Any]:
    ...
    logger.info(
        "Company overview via Alpha Vantage",
        symbol=symbol,
...
async def fetch_news_sentiment(
    ...
    if topics:
        ...
        filter_desc = (
            f"topics={topics}" if not filter_desc else f"{filter_desc}, topics={topics}"
        )
...
async def fetch_top_gainers_losers(mixin: Any) -> dict[str, Any]:
    ...
    data = response.json()
    if not any(
        k in data for k in ["top_gainers", "top_losers", "most_actively_traded"]
    ):
        raise ValueError(
            f"No market movers data available: {mixin._sanitize_response(data)}"
        )
    logger.info(
