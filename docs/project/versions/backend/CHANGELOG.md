...
- Adds parallel-tool/reasoning/usage/error/cancellation coverage and successful real-account checks for all 16 discovered supported models.
- Ships through protected PR #6 (`c00efe6d`), implementation `0719b92`, after full local/hosted, clean-image and browser validation.
...
- Preserves existing providers; passes full local/hosted gates and 3 native browser scenarios, plus clean-image and live-account verification.
- Ships through protected PR #4 (`94a190f2`), implementation `5b097d0`, with private credential persistence verified across image recreation.
...
- **Phase 1 quote tools** (`src/agent/tools/finnhub/quotes.py` + `src/agent/tools/alpha_vantage/quotes.py`): output appends `After-hours: $X.XX (±Y%) vs primary print` or `Pre-market: ...` when the companion is present. The same source-id token (`[FH-Q-...]` / `[YF-Q-...]`) covers both lines (one quote = one citation). 4 tool-body tests in `test_phase1_quote_tools_ext_hours.py`.
- **Phase 1 prompt** (`src/agent/portfolio/phase1_research.py`): one-sentence rule directing the LLM to note the companion verbatim with its source-ID token whenever the move is ≥ ±1% versus the primary.
...
**Numbers:** 41 new tests across 5 backend test files + 8 frontend Vitest cases (49 total). Full pre-existing ext-hours suite still 21/21 green; W3.6 + W3.17 prompt locks still 9/9 green. No regressions in the 9 pre-existing failing tests (verified by stash → run → unstash).
...
- **W3.10 `insider_enrich.py` pure-function bridge** — new module `src/agent/tools/finnhub/insider_enrich.py` connects the existing `finnhub_insider_trades` provider rows (Finnhub / AlphaVantage / yfinance, each in its own dict shape) to the W3.9 `Form4Transaction` records. Three deliverables: `enrich_insider_rows(rows, form4_txs)`, `compute_pct_of_holdings_after(row)`, and `build_last_12mo_summary(form4_txs, anchor_date)`. All helpers are sync, side-effect-free, and tolerant of missing / malformed inputs — a single bad row never kills the enrichment pass. The module is intentionally NOT yet wired into the live `finnhub_insider_trades` tool; that wiring lives behind the SEC EDGAR live-traffic path (W3.13 integration test) so the merge surface stays opt-in and the offline test suite remains hermetic.
- **`enrich_insider_rows(rows, form4_txs)` returns a NEW list** — never mutates the input. Match heuristic: same transaction date AND share count within 1 share. Date is parsed from any of `transactionDate` (Finnhub), `Date` (AlphaVantage), `Start Date` (yfinance), `filingDate` (Finnhub fallback). Share count is parsed from `share` / `Shares` / `change` and tolerates string + comma forms. The 1-share tolerance handles the Finnhub-rounds-to-int vs. Form-4-carries-fractional-RSU-shares mismatch. Match merges `plan_type`, `shares_owned_after`, `plan_adopted_date.isoformat()`, and the derived `pct_of_holdings_after` into the row. Rows without a Form 4 match pass through unchanged. Empty / None `form4_txs` short-circuits to a list of shallow-copied rows.
- **`compute_pct_of_holdings_after(row)` derives shares / shares_owned_after** — returned as a 0..1 ratio (5,000 shares of 100,000 owned → 0.05 = "5% of remaining position"). Returns `None` when either leg is missing, when `shares_owned_after` parses to ≤ 0, or when the value is unparseable. The Phase1 prompt rule treats `None` as "unknown" and skips the bearish framing. The `share` field can be int, float, or comma-formatted string; `shares_owned_after` accepts any value coercible via `float()`.
- **`Last12moSummary` dataclass + `build_last_12mo_summary(form4_txs, anchor_date)`** — symbol-level 365-day aggregate. Fields: `transaction_count`, `plan_breakdown: dict[str, int]` (plan_type → count), `total_shares: float`. The window is `[anchor_date - 365d, anchor_date]` inclusive on both ends; transactions with `transaction_date is None` are skipped. `anchor_date` defaults to `datetime.now(UTC).date()` when not supplied. `Last12moSummary.render()` produces a one-line markdown summary like `3 tx · 2 10b5-1 · 1 discretionary · ~12,500 shares total`; empty input → `"no insider transactions in last 12mo"`. The W3.11 Phase1 prompt rule (discretionary cluster → bearish framing) reads this aggregate to decide framing.
- **`render_enriched_row(row)` markdown line per transaction** — shape `- [date] name: N shares (CODE) · plan=10b5-1 · 5.0% of holdings after`. The trailing two segments only appear when enrichment populated them. Pre-enrichment rows render exactly as the W3.5 baseline did, preserving full back-compat. Recognises both the Finnhub (`name` / `share` / `transactionCode`) and AlphaVantage (`Insider` / `Shares` / `Transaction`) row shapes. `plan_type == unknown` is suppressed from the line so noise-level "unknown" tags don't clutter the rendered output.
- **29 new unit tests** in `tests/test_insider_enrich.py` — covers `enrich_insider_rows` (no-form4, empty-form4, matched-merge of all four enrichment fields, no-mutation, fractional-share tolerance, share-mismatch beyond tolerance, date-mismatch, AV / yfinance row shapes, first-match-wins, unparseable-date passthrough), `compute_pct_of_holdings_after` (happy path, missing-shares / missing-after, zero-after / negative-after → None, comma-string inputs, unparseable-after → None), `Last12moSummary` rendering for empty + populated cases, `build_last_12mo_summary` (window inclusivity at anchor and 365d cutoff, plan_breakdown counts across all three plan types, default-anchor-uses-today smoke test, drops txs with `transaction_date=None`), `render_enriched_row` (W3.5 baseline pre-enrichment / appends `plan=` + `pct%` post-enrichment / unknown plan suppressed / missing pct suppressed / AV row shape). Wave 3 regression sweep (W3.4 + W3.5 + W3.6 + W3.8 + W3.9 + W3.10) = 93/93 green.

Bumps backend 0.27.17 → 0.27.18.
