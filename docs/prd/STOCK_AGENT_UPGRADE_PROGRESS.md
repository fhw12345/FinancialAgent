...
| W2.9 | Numeric derivation `{value, formula, inputs}` + helpers | ✅ | (pending) | derivations.py: Derivation Pydantic model + atr_stop / vol_adjusted_size helpers. TradingDecision gains optional entry/stop/target/size_derivation fields. Cross-validator: derivation.value must match corresponding price within 0.5%. 16 unit tests. |
